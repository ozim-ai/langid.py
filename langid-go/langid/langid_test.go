...
func TestNewInvalidPath(t *testing.T) {
	...
	if err == nil {
		t.Error("Expected error when model directory doesn't exist")
	}
}
...
func TestRank(t *testing.T) {
	...
	// Check that first result is English
	if results[0].Language != "en" {
		t.Errorf("Expected English as top result, got %s", results[0].Language)
	}
}
...
func TestSetLanguages(t *testing.T) {
	...
	// Test with text that should be classified as German
	text = "Hallo Welt"
	language, _ = identifier.Classify(text)
	if language != "de" {
		t.Errorf("Expected German, got %s", language)
	}
...
func TestNormalizeProbs(t *testing.T) {
	...
	if confidence < 0 || confidence > 1 {
		t.Errorf("Expected normalized confidence between 0 and 1, got %f", confidence)
	}
	if language != "en" {
		t.Errorf("Expected English, got %s", language)
	}
...
func TestRankWithNormalization(t *testing.T) {
	...
	for _, result := range results {
		if result.Confidence < 0 || result.Confidence > 1 {
			t.Errorf("Expected normalized confidence between 0 and 1, got %f", result.Confidence)
		}
	}
}
